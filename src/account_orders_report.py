...
ORDERS_PER_PAGE = 40          # rows of order history per continuation page
...
def _render_account(pdf, d):
    ...
    orders = d["orders"]
    ocols = ["Submitted", "Filled", "Side", "Symbol", "FillQty", "Fill px", "Value", "Rlz $", "Unrlz $", "Ret %", "Status"]
    oraw = [10, 10, 5, 7, 6, 8, 8, 8, 8, 8, 8]
    ...
    def _color_orders(tbl, chunk):
        for i, o in enumerate(chunk):
            side = (o.get("side") or "").upper()
            tbl[i + 1, 2].set_text_props(color=(GREEN if side == "BUY" else RED), weight="bold")
            if o.get("rlz") is not None:                       # realized SELL
                tbl[i + 1, 7].set_text_props(color=(GREEN if o["rlz"] >= 0 else RED), weight="bold")
            if o.get("unrlz") is not None:                     # mark-to-market of a held BUY
                tbl[i + 1, 8].set_text_props(color=(GREEN if o["unrlz"] >= 0 else RED), weight="bold")
            if o.get("ret") is not None:
                tbl[i + 1, 9].set_text_props(color=(GREEN if o["ret"] >= 0 else RED), weight="bold")
            if o.get("status") != "filled":
                tbl[i + 1, 10].set_text_props(color=GREY, style="italic")
    ...
    while rest or page == 1:                                  # always emit at least one (possibly empty) page
        ...
        _, ot = _table(fig, [0.06, 0.05, 0.88, 0.84], ocols, [_orow(o) for o in chunk], ow, fontsize=6.5)
        if ot is not None:
            _color_orders(ot, chunk)
        fig.text(0.06, 0.92, "Rlz = realized on each SELL (vs running avg cost); Unrlz = each held BUY marked "
                 "to current price; Ret %% pairs with whichever applies.  Σ realized (sells) = %s.  Times in CT."
                 % _money(realized_sum), fontsize=7.5, color="#555")
        pdf.savefig(fig); plt.close(fig)
        if not rest:
            break
        page += 1
